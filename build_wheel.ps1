<#
.SYNOPSIS
    Build the wc_cuda Python wheel using uv + maturin.
...
.DESCRIPTION
    ...
    wc_cuda is a native PyO3 extension, so the produced wheel is
    platform-specific (x64 Windows). It targets the abi3-py310 stable
    ABI, so a single wheel works for CPython 3.10+.
...
if ($Develop) {
    ...
    Write-Host "`nInstalled wc_cuda into .venv (editable/native)." -ForegroundColor Green
}
else {
