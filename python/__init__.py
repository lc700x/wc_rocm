...
class _DX11ToPyTorchBridge:
    def __init__(self, texture_ptr, width, height, device_id=0, reuse_output_buffer=False, output_buffer_count=3):
        self.width = width
        self.height = height
        self.device_id = device_id
        self.device = torch.device(f"cuda:{device_id}")
        self.reuse_output_buffer = bool(reuse_output_buffer)
        self.output_buffer_count = max(2, int(output_buffer_count))
        self._buffer_pool = []
        self._pool_index = 0
    ...
    def _acquire_target_tensor(self):
        if not self.reuse_output_buffer:
            return torch.empty((self.height, self.width, 4), dtype=torch.uint8, device=self.device)

        if not self._buffer_pool:
            self._buffer_pool = [
                torch.empty((self.height, self.width, 4), dtype=torch.uint8, device=self.device)
                for _ in range(self.output_buffer_count)
            ]

        tensor = self._buffer_pool[self._pool_index]
        self._pool_index = (self._pool_index + 1) % self.output_buffer_count
        return tensor

    def update(self, stream):
        """Copies from DX11 texture to a CUDA tensor on a dedicated stream."""
        target_tensor = self._acquire_target_tensor()
            ...
            while self._running:
                ...
                if res:
                    ...
                    if self._bridge is None or self._bridge.width != w or self._bridge.height != h:
                        self._bridge = _DX11ToPyTorchBridge(
                            gpu_frame.texture_ptr,
                            w,
                            h,
                            self.device_id,
                            reuse_output_buffer=self._reuse_output_buffer,
                            output_buffer_count=self._output_buffer_count,
                        )

                    # Queue the copy on the dedicated stream.
                    current_stream = torch.cuda.current_stream(device=self.device_id)
                    if self._reuse_output_buffer:
                        # Don't overwrite a pooled buffer until any consumer GPU
                        # work queued on the current stream has finished.
                        self._stream.wait_stream(current_stream)
                    tensor = self._bridge.update(self._stream)
                    if tensor is not None:
                        # Make the current stream wait without blocking the CPU,
                        # so consumer ops (including .cpu()) see a completed copy.
